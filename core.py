...
class environment:
    ...
    def gen_sleep_samples_rand_angles(self, s0, Tmax, alpha):
        ...
        for t in range(1,Tmax):
            noise = rnd.randn(2,)
            s_sleep[:,t] = (alpha.dot(self.T.dot(Psi_s)).squeeze() + self.sig_s * noise/np.linalg.norm(noise) )
    ...
    def gen_sleep_samples(self, s0, Tmax, alpha):
        ...
        for t in range(1,Tmax):
            s_sleep[:,t] = (alpha.dot(self.T.dot(Psi_s)).squeeze() + self.sig_s * rnd.randn(2,))
            Psi_s = np.concatenate([ self.Psi_fun(s_sleep[:,t].reshape(-1,1)), np.ones((1,1))])
    ...
    def Psi_fun(self, s):
        ...
        rbf = 1./(np.sqrt(2)*sig_psi) * exp(-1./(sig_psi**2) * np.sum((s[np.newaxis,:,:] - grid_coord[:,:,np.newaxis])**2, axis=1) ) 
    ...
    def wake_sleep(self, alpha, Niter, eps, tau):
        for it in tqdm(range(Niter)):
            ...
            # run filtering on real wake observations
